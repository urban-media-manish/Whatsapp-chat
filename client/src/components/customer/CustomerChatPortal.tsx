...
export const CustomerChatPortal: React.FC = () => {
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-[#efeae2] dark:bg-[#0b141a] transition-colors duration-300">
      {/* ── WhatsApp Green + iOS Translucent Header Bar ── */}
      <div className="bg-[#008069] dark:bg-[#202c33] px-4 py-3 border-b border-black/10 dark:border-white/[0.08] flex items-center justify-between shadow-sm z-20 text-white select-none transition-colors">
        <div className="flex items-center gap-3">
          <div className="relative cursor-pointer flex items-center gap-3" onClick={() => setIsEditingProfile(true)}>
            <div className="relative">
              ...
              />
              <span className="absolute bottom-0 right-0 w-3 h-3 bg-[#25D366] rounded-full border-2 border-[#008069] dark:border-[#202c33]" />
            </div>
            <div>
              <div className="flex items-center gap-1.5">
                <h2 className="text-sm md:text-base font-bold text-white tracking-tight">
                  {customerConversation?.assignedAgent?.name || "WhatsApp Support"}
                </h2>
              ...
              <p className="text-[11px] text-emerald-100 dark:text-emerald-400 font-medium">
                {isAgentTyping ? (
                  <span className="text-amber-200 font-semibold animate-pulse">typing...</span>
                ) : (
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            className="w-9 h-9 flex items-center justify-center rounded-xl bg-white/15 hover:bg-white/25 text-white transition-all active:scale-90"
          >
      ...
      {showSearch && (
        <div className="bg-white/95 dark:bg-[#1c1c1e]/95 backdrop-blur-2xl px-4 py-2.5 border-b border-black/[0.06] dark:border-white/[0.08] shadow-md z-10 flex flex-wrap items-center justify-between gap-3 animate-in slide-in-from-top-2 duration-200">
          <div className="flex-1 min-w-[200px] flex items-center gap-2 bg-[#f0f2f5] dark:bg-[#2c2c2e] px-3.5 py-1.5 rounded-xl border border-black/[0.05] dark:border-white/[0.08]">
            ...
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery('')} className="text-[#8696a0] hover:text-[#111b21] dark:hover:text-white">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#00a884]/15 hover:bg-[#00a884]/25 text-[#00a884] dark:text-emerald-400 text-xs font-semibold transition-all active:scale-95"
            >
            ...
            <button
              ...
              title="Export as PDF"
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#00a884] hover:bg-[#008f70] text-white text-xs font-semibold transition-all active:scale-95 shadow-sm"
            >
              <Download className="w-3.5 h-3.5" />
              <span>Export PDF</span>
            </button>
            <button onClick={() => setShowSearch(false)} className="p-1 rounded-lg text-[#8696a0] hover:text-[#111b21] dark:hover:text-white">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
      ...
      {showVoiceCall && (
        <VoiceCallModal
          contactName={customerConversation?.assignedAgent?.name || "WhatsApp Support"}
      ...
      {isEditingProfile && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#202c33] p-6 rounded-[24px] max-w-sm w-full relative shadow-2xl border border-black/[0.06] dark:border-white/[0.1] animate-in zoom-in-95 duration-200">
            <button onClick={() => setIsEditingProfile(false)} className="absolute top-4 right-4 text-[#8696a0] hover:text-[#111b21] dark:hover:text-white">
              <X className="w-5 h-5" />
            </button>
            <h3 className="text-base font-bold text-[#111b21] dark:text-[#e9edef] mb-4">Chat Profile</h3>
            <form onSubmit={handleUpdateProfile} className="space-y-3">
              <div>
                <label className="block text-[11px] font-semibold text-[#8696a0] mb-1">Your Name</label>
                <input
                  type="text"
                  value={nameInput}
                  onChange={(e) => setNameInput(e.target.value)}
                  placeholder="Full Name"
                  className="w-full bg-[#f0f2f5] dark:bg-[#2a3942] border border-black/[0.06] dark:border-white/[0.08] rounded-xl px-3.5 py-2 text-sm text-[#111b21] dark:text-[#e9edef] outline-none focus:border-[#00a884]"
                />
              </div>
              <div>
                <label className="block text-[11px] font-semibold text-[#8696a0] mb-1">Phone Number</label>
                <input
                  type="tel"
                  value={phoneInput}
              ...
              <button type="submit" className="w-full mt-2 bg-[#00a884] hover:bg-[#008f70] text-white py-2.5 rounded-xl font-semibold text-sm flex items-center justify-center gap-1.5 shadow-sm active:scale-98 transition-all">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 chat-wallpaper">
        ...
        {/* ════ WHATSAPP PROFILE CARD (iOS Card Style) ════ */}
        <div className="my-3 max-w-sm mx-auto">
          <div className="bg-white/90 dark:bg-[#202c33]/90 backdrop-blur-2xl rounded-[24px] p-5 shadow-sm text-center border border-black/[0.06] dark:border-white/[0.08] transition-all">
            <div className="relative w-16 h-16 mx-auto mb-3">
              ...
              />
              <span className="absolute bottom-0 right-0 w-4 h-4 bg-[#25D366] rounded-full border-2 border-white dark:border-[#202c33]" />
            </div>
            ...
            <p className="text-xs text-[#667781] dark:text-[#8696a0] mb-4 font-medium">
              Official WhatsApp Business Live Desk
            </p>
            ...
            <div className="grid grid-cols-3 gap-2 mb-4">
              <button
                ...
                className="bg-[#00a884]/10 hover:bg-[#00a884]/20 text-[#00a884] dark:text-emerald-400 p-2.5 rounded-2xl flex flex-col items-center justify-center gap-1 text-xs font-semibold transition-all active:scale-95"
              >
                <Search className="w-4 h-4" />
                <span>Search</span>
              </button>
            ...
            {/* Security Badges Row */}
            <div className="flex items-center justify-center gap-4 text-[11px] font-semibold text-[#00a884] dark:text-emerald-400 pt-3 border-t border-black/[0.05] dark:border-white/[0.06]">
        ...
        <div className="flex justify-center my-3">
          <span className="bg-white/80 dark:bg-[#202c33]/80 text-[#667781] dark:text-[#8696a0] text-[11px] font-semibold px-3.5 py-1 rounded-full shadow-xs backdrop-blur-md">
            Today
          </span>
        </div>
        ...
        {inlineStep === 'name' && (
          <div className="flex justify-center my-4">
            <div className="bg-white/95 dark:bg-[#202c33]/95 backdrop-blur-2xl border border-black/[0.06] dark:border-white/[0.1] rounded-[24px] p-5 shadow-xl max-w-sm w-full animate-in fade-in zoom-in-95 duration-200">
              <div className="flex items-center gap-2.5 mb-3">
                <div className="w-8 h-8 rounded-full bg-[#00a884]/15 text-[#00a884] flex items-center justify-center flex-shrink-0">
                  <Sparkles className="w-4 h-4" />
                </div>
                <div>
                  <h4 className="text-sm font-bold text-[#111b21] dark:text-[#e9edef]">Welcome to WhatsApp Support</h4>
                  <p className="text-[11px] text-[#86868b]">Step 1 of 2 • Enter your name</p>
                </div>
              </div>

              <form onSubmit={handleStep1Submit} className="space-y-3">
                <div className="flex items-center gap-2.5 bg-[#f0f2f5] dark:bg-[#2a3942] border border-black/[0.06] dark:border-white/[0.08] rounded-xl px-3.5 py-2.5 focus-within:border-[#00a884] focus-within:ring-2 focus-within:ring-[#00a884]/20 transition-all">
                  ...
                  <input
                    ...
                    required
                    autoFocus
                    className="flex-1 bg-transparent text-sm text-[#111b21] dark:text-[#e9edef] outline-none placeholder-[#86868b]"
                  />
                </div>
                <button
                  type="submit"
                  disabled={!nameInput.trim()}
                  className="w-full bg-[#00a884] hover:bg-[#008f70] text-white py-2.5 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 disabled:opacity-40 shadow-sm active:scale-98 transition-all"
                >
          ...
          <div className="flex justify-center my-4">
            <div className="bg-white/95 dark:bg-[#202c33]/95 backdrop-blur-2xl border border-black/[0.06] dark:border-white/[0.1] rounded-[24px] p-5 shadow-xl max-w-sm w-full animate-in fade-in zoom-in-95 duration-200">
              <div className="flex items-center gap-2.5 mb-3">
                ...
                <div>
                  <h4 className="text-sm font-bold text-[#111b21] dark:text-[#e9edef]">Nice to meet you, {savedName}!</h4>
              ...
              <form onSubmit={handleStep2Submit} className="space-y-3">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="w-1/3 bg-black/[0.05] dark:bg-white/[0.08] text-[#111b21] dark:text-[#e9edef] py-2.5 rounded-xl text-xs font-semibold hover:bg-black/[0.08] transition-all"
                  >
                    ← Back
                  </button>
                  <button
                    ...
                    className="w-2/3 bg-[#00a884] hover:bg-[#008f70] text-white py-2.5 rounded-xl text-sm font-semibold flex items-center justify-center gap-1.5 disabled:opacity-40 shadow-sm active:scale-98 transition-all"
                  >
        ...
        {isAgentTyping && (
          <div className="flex items-center gap-1.5 bg-white/90 dark:bg-[#202c33]/90 backdrop-blur-xl px-3.5 py-2 rounded-full w-fit shadow-xs border border-black/[0.04] dark:border-white/[0.06]">
            ...
            <span className="text-[11px] font-medium text-[#00a884] ml-1">Agent is typing...</span>
          </div>
        )}
      ...
      {inlineStep === 'completed' && customerSession && customerConversation ? (
        ...
        <div className="bg-white/80 dark:bg-[#202c33]/80 backdrop-blur-xl px-4 py-3 border-t border-black/[0.06] dark:border-white/[0.08] flex items-center justify-center text-xs text-[#86868b] font-medium">
